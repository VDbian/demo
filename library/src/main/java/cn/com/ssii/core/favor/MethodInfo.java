    ...
    private void parseMethodAnnotations() {
        ...
        if (responseType == ResponseType.OBSERVABLE) {
            ...
            if (responseObjectType == String.class) {
                ...
                rxPref = rx.getObject(key, defaultValues[0] == null ? null : defaultValues[0], new SerializableAdapter<>(sp, key));
            } else {
...
//            }
    ...
    private class SerializableAdapter<T extends Serializable> implements Preference.Converter<T> {

        private SharedPreferences sp;
        private String key;

        public SerializableAdapter(SharedPreferences sp, String key) {
            this.sp = sp;
            this.key = key;
        }
        ...
        public T deserialize(@NonNull String serialized) {
            return (T) new Taste.SerializableTaste(sp, key, null).get();
        }

        @NonNull
        @Override
        public String serialize(@NonNull T value) {
            return new Taste.SerializableTaste(null, null, null).getSerialized(value);
        }
    }
}
